...
test_that("scalar vars work", {
  ...
  expect_equal(weighted_var(x, rep(1, 10)), var(x))
  w2 <- w / sum(w)
  expect_equal(weighted_var(x, w), (sum(w2 * x^2) - sum(w2 * x)^2) / (1 - sum(w2^2)))
  expect_equal(weighted_var(x, w, method = "moment"), sum(w2 * x^2) - sum(w2 * x)^2)
  w2 <- w[1:2] / sum(w[1:2])
  x2 <- x[1:2]
  expect_equal(running_weighted_var(x, w)[2], (sum(w2 * x2^2) - sum(w2 * x2)^2) / (1 - sum(w2^2)))
  expect_equal(running_var(x)[1:2], c(0, var(x[1:2])))
...
test_that("vector vars work", {
  ...
  expect_equal(weighted_var(x, rep(1, 3)), var(x))
  w2 <- w / sum(w)
  expect_equal(weighted_var(x, w)[1, 1], (sum(w2 * x[, 1]^2) - sum(w2 * x[, 1])^2) / (1 - sum(w2^2)))
