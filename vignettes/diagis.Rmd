...
$$
In case where $x$ can contain several identical values, these elements should be combined (and corresponding weights added together), but for a typical setting where variables $x$ are samples from a continuous distribution, the probability of this zero. Also, for the weighted posterior samples from the IS-MCMC as implemented in `bssm`, identical values due to rejections in MCMC are automatically combined in the jump chain representation [@helske-vihola].
